...
contract FundIssuer is ERC1820Client, IERC1400TokensRecipient, ERC1820Implementer {
    ...
    bytes32 internal constant ORDER_SUBSCRIPTION_FLAG = 0xcccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc;
    bytes32 internal constant ORDER_PAYMENT_FLAG = 0xdddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd;
    ...
    modifier onlyTokenController(address tokenAddress) {
        require(_tokenController(msg.sender, tokenAddress), 'Sender is not a token controller.');
        _;
    }
    ...
    function tokensReceived(
        ...
        if (flag == ORDER_SUBSCRIPTION_FLAG) {
            ...
            _subscribe(from, assetAddress, assetClass, orderData, true, erc1400TokenData);
     ...
     */
    function _startNewCycle(address assetAddress, bytes32 assetClass) internal returns (uint256) {
        AssetRules storage rules = _assetRules[assetAddress][assetClass];
        require(rules.defined, 'Rules not defined for this asset');
    ...
    function subscribe(
        ...
        return _subscribe(msg.sender, assetAddress, assetClass, orderData, executePaymentAtSubscription, new bytes(0));
    }
    ...
    function cancelOrder(uint256 orderIndex) external {
        ...
        require(order.state == OrderState.Subscribed || order.state == OrderState.Paid, 'Only subscribed or paid orders can be cancelled'); // This also checks if the order exists. Otherwise, we would have "order.state == OrderState.Undefined"
    ...
    function rejectOrder(uint256 orderIndex, bool rejected) external {
        ...
        require(
            order.state == OrderState.Subscribed || order.state == OrderState.Paid || order.state == OrderState.Rejected,
        ...
        require(_tokenController(msg.sender, cycle.assetAddress), 'Sender is not a token controller.');
     ...
     */
    function valuate(uint256 cycleIndex, uint256 assetValue, uint256 reverseAssetValue) external {
     ...
     * @param bypassPayment Bypass payment (in case payment has been performed off-chain)
     */
    function batchExecutePaymentsAsController(uint256[] calldata orderIndexes, bool bypassPayment) external {
     ...
     * @param bypassPayment Bypass payment (in case payment has been performed off-chain)
     */
    function _executePayment(uint256 orderIndex, bytes memory erc1400TokenData, bool bypassPayment) internal {
        Order storage order = _orders[orderIndex];
        Cycle storage cycle = _cycles[order.cycleIndex];

        require(order.state == OrderState.Subscribed || order.state == OrderState.UnpaidSettled, 'Order is neither in state Subscribed, nor UnpaidSettled'); // This also checks if the order exists. Otherwise, we would have "order.state == OrderState.Undefined"
        ...
        if (!bypassPayment) {
            if (cycle.paymentType == Payment.ETH) {
                ...
                (address erc1400TokenAddress, bytes32 erc1400TokenPartition, uint256 erc1400PaymentValue) = abi.decode(
                    erc1400TokenData,
    ...
    function _getTotalSettledForCycle(uint256 cycleIndex) internal view returns (uint256, uint256, bool) {
        ...
        for (uint i = 0; i < _cycleOrders[cycleIndex].length; i++) {
            ...
            } else if (order.state != OrderState.Cancelled && order.state != OrderState.Rejected) {
    ...
    function _tokenController(address sender, address assetAddress) internal view returns (bool) {
        if (sender == Ownable(assetAddress).owner() || _isTokenController[assetAddress][sender]) {
            return true;
        } else {
            return false;
        }
    ...
    function getAssetRules(
        address assetAddress,
        bytes32 assetClass
    ) external view returns (uint256, uint256, uint256, uint256, Payment, address, bytes32, address, bool) {
        AssetRules storage rules = _assetRules[assetAddress][assetClass];
        return (
    ...
    function getAssetValueRules(address assetAddress, bytes32 assetClass) external view returns (AssetValue, uint256, uint256) {
        AssetRules storage rules = _assetRules[assetAddress][assetClass];
        return (rules.assetValueType, rules.assetValue, rules.reverseAssetValue);
    }
    ...
    function getCycleAssetValue(uint256 index) external view returns (AssetValue, uint256, uint256) {
        Cycle storage cycle = _cycles[index];
        return (cycle.assetValueType, cycle.assetValue, cycle.reverseAssetValue);
    }
    ...
    function getOrder(uint256 index) external view returns (uint256, address, uint256, uint256, OrderType, OrderState) {
        ...
        return (order.cycleIndex, order.investor, order.value, order.amount, order.orderType, order.state);
    }
