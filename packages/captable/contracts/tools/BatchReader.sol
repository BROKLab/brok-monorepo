...
abstract contract IERC1400TokensValidatorExtended is IExtensionTypes {
    ...
    function retrieveTokenSetup(address token) external view virtual returns (CertificateValidation, bool, bool, bool, bool, address[] memory);

    function spendableBalanceOfByPartition(address token, bytes32 partition, address account) external view virtual returns (uint256);

    function isAllowlisted(address token, address account) public view virtual returns (bool);

    function isBlocklisted(address token, address account) public view virtual returns (bool);
}
...
contract BatchReader is IExtensionTypes, ERC1820Client, ERC1820Implementer {
     ...
     */
    function batchTokenSuppliesInfos(
        address[] calldata tokens
    ) external view returns (uint256[] memory, uint256[] memory, bytes32[] memory, uint256[] memory, uint256[] memory, bytes32[] memory) {
        uint256[] memory batchTotalSupplies = new uint256[](tokens.length);
        for (uint256 j = 0; j < tokens.length; j++) {
            batchTotalSupplies[j] = IERC20(tokens[j]).totalSupply();
        }

        (uint256[] memory totalPartitionsLengths, bytes32[] memory batchTotalPartitions_, uint256[] memory batchPartitionSupplies) = batchTotalPartitions(
            tokens
        );
     ...
     */
    function batchTokenRolesInfos(
        address[] calldata tokens
    ) external view returns (address[] memory, uint256[] memory, address[] memory, uint256[] memory, address[] memory) {
     ...
     */
    function batchTokenExtensionSetup(
        address[] calldata tokens
    ) external view returns (address[] memory, CertificateValidation[] memory, bool[] memory, bool[] memory, bool[] memory, bool[] memory) {
        (
     ...
     */
    function batchTokenExtensionSetup1(
        address[] memory tokens
    ) public view returns (address[] memory, CertificateValidation[] memory, bool[] memory, bool[] memory) {
        address[] memory batchTokenExtension = new address[](tokens.length);
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (batchTokenExtension[i] != address(0)) {
                (CertificateValidation certificateActivated, bool allowlistActivated, bool blocklistActivated, , , ) = IERC1400TokensValidatorExtended(
                    batchTokenExtension[i]
    ...
    function batchTokenExtensionSetup2(address[] memory tokens) public view returns (bool[] memory, bool[] memory) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (batchTokenExtension[i] != address(0)) {
                (, , , bool granularityByPartitionActivated, bool holdsActivated, ) = IERC1400TokensValidatorExtended(batchTokenExtension[i])
                    .retrieveTokenSetup(tokens[i]);
    ...
    function batchERC1400Balances(
    ...
    ) external view returns (uint256[] memory, uint256[] memory, uint256[] memory, bytes32[] memory, uint256[] memory, uint256[] memory) {
        ...
        (
            ...
            bytes32[] memory batchTotalPartitions_,
    ...
    ) public view returns (uint256[] memory, bytes32[] memory, uint256[] memory) {
        ...
        for (uint256 i = 0; i < tokenHolders.length; i++) {
            ...
            for (uint256 j = 0; j < tokens.length; j++) {
                for (uint256 k = 0; k < totalPartitionsLengths[j]; k++) {
                    batchBalanceOfByPartitionResponse[i * batchTotalPartitions_.length + counter] = IERC1400(tokens[j]).balanceOfByPartition(
                        batchTotalPartitions_[counter],
                        tokenHolders[i]
                    );
                    counter++;
                }
            ...
            for (uint256 j = 0; j < tokens.length; j++) {
                ...
                for (uint256 k = 0; k < totalPartitionsLengths[j]; k++) {
                    if (tokenExtension != address(0)) {
                        batchSpendableBalanceOfByPartitionResponse[i * batchTotalPartitions_.length + counter] = IERC1400TokensValidatorExtended(tokenExtension)
                            .spendableBalanceOfByPartition(tokens[j], batchTotalPartitions_[counter], tokenHolders[i]);
                    } else {
                        batchSpendableBalanceOfByPartitionResponse[i * batchTotalPartitions_.length + counter] = IERC1400(tokens[j]).balanceOfByPartition(
                            batchTotalPartitions_[counter],
                            tokenHolders[i]
                        );
                    }
