...
contract ERC1400TokensChecker is IERC1400TokensChecker, ERC1820Client, ERC1820Implementer {
    ...
    function _canTransferByPartition(
        ...
        if (!IERC1400(msg.sender).isOperatorForPartition(partition, operator, from)) return (hex'58', '', partition); // 0x58	invalid operator (transfer agent)
        ...
        IERC1400TokensValidator.ValidateData memory vdata = IERC1400TokensValidator.ValidateData(
            msg.sender,
            payload,
            partition,
            operator,
        ...
        if ((hookImplementation != address(0)) && !IERC1400TokensValidator(hookImplementation).canValidate(vdata)) return (hex'54', '', partition); // 0x54	transfers halted (contract paused)

        uint256 granularity = IERC1400Extended(msg.sender).granularity();
        if (!(value.div(granularity).mul(granularity) == value)) return (hex'50', '', partition); // 0x50	transfer failure
