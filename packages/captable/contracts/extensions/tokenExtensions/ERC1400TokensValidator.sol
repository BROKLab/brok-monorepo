...
import '../../interfaces/IHoldableERC1400TokenExtension.sol';
    ...
    IHoldableERC1400TokenExtension
{
    ...
    bytes4 internal constant ERC20_TRANSFER_ID = bytes4(keccak256('transfer(address,uint256)'));
    bytes4 internal constant ERC20_TRANSFERFROM_ID = bytes4(keccak256('transferFrom(address,address,uint256)'));
    ...
    modifier onlyTokenController(address token) {
        require(msg.sender == token || msg.sender == Ownable(token).owner() || _isTokenController[token][msg.sender], 'Sender is not a token controller.');
        _;
    }
    ...
    modifier onlyPauser(address token) override {
        require(
            msg.sender == token || msg.sender == Ownable(token).owner() || _isTokenController[token][msg.sender] || isPauser(token, msg.sender),
    ...
    modifier onlyCertificateSigner(address token) override {
        require(
            msg.sender == token || msg.sender == Ownable(token).owner() || _isTokenController[token][msg.sender] || isCertificateSigner(token, msg.sender),
    ...
    modifier onlyAllowlistAdmin(address token) override {
        require(
            msg.sender == token || msg.sender == Ownable(token).owner() || _isTokenController[token][msg.sender] || isAllowlistAdmin(token, msg.sender),
    ...
    modifier onlyBlocklistAdmin(address token) override {
        require(
            msg.sender == token || msg.sender == Ownable(token).owner() || _isTokenController[token][msg.sender] || isBlocklistAdmin(token, msg.sender),
    ...
    function tokensToValidate(
        ...
        {
            (bool canValidateCertificateToken, CertificateValidation certificateControl, bytes32 salt) = _canValidateCertificateToken(
                msg.sender,
                payload,
                operator,
...

        {
            ...
            if (_holdsActivated[msg.sender] && holdId != '') {
                ...
                _setHoldToExecuted(msg.sender, executableHold, holdId, value, executableHold.value, '');
            }
        }
    }
    ...
    ) internal view returns (bool, CertificateValidation, bytes32) {
        if (
            ...
            if (_certificateActivated[token] == CertificateValidation.SaltBased) {
                (bool valid, bytes32 salt) = _checkSaltBasedCertificate(token, operator, payload, certificate);
                if (valid) {
                ...
                if (_checkNonceBasedCertificate(token, operator, payload, certificate)) {
    ...
    function _canValidateAllowlistAndBlocklistToken(
        ...
        if (!_functionSupportsCertificateValidation(payload) || _certificateActivated[token] == CertificateValidation.None) {
     ...
     * @return 'true' if the token transfer can be validated, 'false' if not.
     */
    function _canValidateGranularToken(address token, bytes32 partition, uint value) internal view returns (bool) {
        if (_granularityByPartitionActivated[token]) {
            if (_granularityByPartition[token][partition] > 0 && !_isMultiple(_granularityByPartition[token][partition], value)) {
                return false;
            }
        }
     ...
     * @return 'true' if the token transfer can be validated, 'false' if not.
     */
    function _canValidateHoldableToken(address token, bytes32 partition, address operator, address from, address to, uint value) internal view returns (bool) {
    ...
    function preHoldFor(
        ...
        return _createHold(token, holdId, address(0), recipient, notary, partition, value, _computeExpiration(timeToExpiration), secretHash, certificate);
    }
    ...
    function preHoldForWithExpirationDate(
        ...
        return _createHold(token, holdId, address(0), recipient, notary, partition, value, expiration, secretHash, certificate);
    }
    ...
    function hold(
        ...
        return _createHold(token, holdId, msg.sender, recipient, notary, partition, value, _computeExpiration(timeToExpiration), secretHash, certificate);
    }
    ...
    function holdWithExpirationDate(
        ...
        return _createHold(token, holdId, msg.sender, recipient, notary, partition, value, expiration, secretHash, certificate);
    }
    ...
    function holdFrom(
        ...
        return _createHold(token, holdId, sender, recipient, notary, partition, value, _computeExpiration(timeToExpiration), secretHash, certificate);
    }
    ...
    function holdFromWithExpirationDate(
        ...
        return _createHold(token, holdId, sender, recipient, notary, partition, value, expiration, secretHash, certificate);
    }
    ...
    function _createHold(
        ...
        require(_canHoldOrCanPreHold(token, msg.sender, sender, certificate), 'A hold can only be created with adapted authorizations');

        if (sender != address(0)) {
            // hold (tokens already exist)
            require(
                value <= _spendableBalanceOfByPartition(token, partition, sender),
        ...
        if (sender != address(0)) {
            ...
            (bytes32 holdHash, ) = _retrieveHoldHashId(token, newHold.partition, newHold.notary, newHold.sender, newHold.recipient, newHold.value);

            _holdIds[holdHash] = holdId;
        }

        emit HoldCreated(token, holdId, partition, sender, recipient, notary, value, expiration, secretHash);
    ...
    function _releaseHold(address token, bytes32 holdId) internal returns (bool) {
        ...
        require(
            _isExpired(releasableHold.expiration) || (msg.sender == releasableHold.notary) || (msg.sender == releasableHold.recipient),
    ...
    function _renewHold(address token, bytes32 holdId, uint256 expiration, bytes memory certificate) internal returns (bool) {
        ...
        require(_canHoldOrCanPreHold(token, msg.sender, renewableHold.sender, certificate), 'A hold can only be renewed with adapted authorizations');

        uint256 oldExpiration = renewableHold.expiration;
        renewableHold.expiration = expiration;

        emit HoldRenewed(token, holdId, renewableHold.notary, oldExpiration, expiration);
    ...
    function executeHold(address token, bytes32 holdId, uint256 value, bytes32 secret) external override {
        _executeHold(token, holdId, msg.sender, value, secret, false);
    }
    ...
    function executeHoldAndKeepOpen(address token, bytes32 holdId, uint256 value, bytes32 secret) external {
        _executeHold(token, holdId, msg.sender, value, secret, true);
    }

    /**
     * @dev Execute hold.
     */
    function _executeHold(address token, bytes32 holdId, address operator, uint256 value, bytes32 secret, bool keepOpenIfHoldHasBalance) internal {
        ...
        if (canExecuteHold) {
            if (keepOpenIfHoldHasBalance && ((executableHold.value - value) > 0)) {
                _setHoldToExecutedAndKeptOpen(token, executableHold, holdId, value, value, secret);
            } else {
                _setHoldToExecuted(token, executableHold, holdId, value, executableHold.value, secret);
            }
    ...
    function _setHoldToExecuted(
        ...
        emit HoldExecuted(token, holdId, executableHold.notary, executableHold.value, value, secret);
    }
    ...
    function _setHoldToExecutedAndKeptOpen(
        ...
        emit HoldExecutedAndKeptOpen(token, holdId, executableHold.notary, executableHold.value, value, secret);
    }
    ...
    function _increaseHeldBalance(address token, Hold storage executableHold, bytes32 /*holdId*/) private {
        ...
        _heldBalanceByPartition[token][executableHold.sender][executableHold.partition] = _heldBalanceByPartition[token][executableHold.sender][
    ...
    function _retrieveHoldHashId(
        ...
        bytes32 holdHash = keccak256(abi.encodePacked(token, partition, sender, recipient, notary, value));
    ...
    function _holdCanBeExecutedAsSecretHolder(Hold storage executableHold, uint value, bytes32 secret) internal view returns (bool) {
        if (_checkSecret(executableHold, secret) && _holdCanBeExecuted(executableHold, value)) {
            return true;
        } else {
            return false;
        }
    ...
    function _holdCanBeExecutedAsNotary(Hold storage executableHold, address operator, uint value) internal view returns (bool) {
        if (executableHold.notary == operator && _holdCanBeExecuted(executableHold, value)) {
            return true;
        } else {
            return false;
        }
    ...
    function _canHoldOrCanPreHold(address token, address operator, address sender, bytes memory certificate) internal returns (bool) {
        (bool canValidateCertificate, CertificateValidation certificateControl, bytes32 salt) = _canValidateCertificateToken(
            token,
            msg.data,
            operator,
            certificate
    ...
    function _checkNonceBasedCertificate(
        ...
        if (v == 27 || v == 28) {
            ...
            // Pack and hash
            bytes memory pack = abi.encodePacked(msgSender, token, payloadWithoutCertificate, e, _usedCertificateNonce[token][msgSender]);
            bytes32 hash = keccak256(abi.encodePacked(DomainAware(token).generateDomainSeparator(), keccak256(pack)));
    ...
    function _checkSaltBasedCertificate(
        ...
        if (v == 27 || v == 28) {
            ...
            // Pack and hash
            bytes memory pack = abi.encodePacked(msgSender, token, payloadWithoutCertificate, e, salt);
