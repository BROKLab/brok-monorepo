...
export async function _editEncumbrance(this: SDK, shareholderCeramicID: CeramicID, updatedEncumbrance: Partial<Encumbrance>) {
  try {
    const isFagsystem = await this.blockchain
      .capTableRegistryContract()
      .hasRole(ethers.utils.solidityKeccak256(['string'], ['FAGSYSTEM']), this.blockchain.signer.address);
  ...
  if (!oldShareholderRes.value.encumbrance) {
    return err('This shareholder does not have any encumbrance.');
  }
  const updatedShareholder = await this.ceramic.updateShareholder(shareholderCeramicID, {
    ...oldShareholderRes.value,
    encumbrance: {
      ...oldShareholderRes.value.encumbrance,
      ...updatedEncumbrance,
    },
  });
  if (updatedShareholder.isErr()) {
...
export async function _deleteEncumbrance(this: SDK, shareholderCeramicID: CeramicID) {
  try {
    const isFagsystem = await this.blockchain
      .capTableRegistryContract()
      .hasRole(ethers.utils.solidityKeccak256(['string'], ['FAGSYSTEM']), this.blockchain.signer.address);
