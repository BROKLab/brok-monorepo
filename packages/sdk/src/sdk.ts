...
export class SDK {
	...
	): Promise<(OperationResult & TransferRequest)[]> {
	...
	async updateShareholder(
		capTableAddress: CapTableEthereumId,
		shareholder: Partial<Shareholder>,
	): Promise<Shareholder> {
	...
	async kapitalforhoyselseNyeAksjer(
		capTableAddress: CapTableEthereumId,
		transfers: IssueInput[],
	): Promise<(OperationResult & IssueRequest)[]> {
	...
	private static async initWallet(rpc: string, secret: string) {
		try {
			...
			const signer = ethers.utils.isHexString(secret)
				? new Wallet(secret, provider)
				: Wallet.fromMnemonic(secret).connect(provider);
			return ok(signer);
		} catch (e) {
